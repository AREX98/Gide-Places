...
//hasta aqui ....
if(!$nitCliente=="" && !$nameCliente=="" && !$apeCliente=="" && !$dirCliente==""&& !$fullnameCliente==""){
    $verificar=  ejecutarSQL::consultar("select * from Usuarios where idUsuario='".$nitCliente."'");
    $verificaltotal = mysql_num_rows($verificar);
    if($verificaltotal<=0){
        if(consultasSQL::InsertSQL("Usuarios", "idUsuarios, nombreUsuario, apellidoUsuario, edadUsuario, emailUsuario", "'$nitCliente','$nameCliente','$fullnameCliente','$apeCliente', '$passCliente'")){
